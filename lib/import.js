import { CsvLineStream } from '@pirxpilot/csv-parse';
...
function conformStop(s) {
  if (s.lat && s.lon) {
    s.coordinates = {
      ...
      lon: Number.parseFloat(s.lon)
    };
    delete s.lat;
    delete s.lon;
  }
  if (s.duration) {
    const duration = Number.parseFloat(s.duration);
...
export default async function parseCsv(file) {
  ...
  let header = false;
  await file.pipeThrough(parser).pipeTo(
    new WritableStream({
      write: row => {
        if (!header) {
          header = columns(row);
  ...
  function columns(row) {
    ...
    if (!(row.length < 3 || Number.isNaN(Number.parseFloat(row[0])) || Number.isNaN(Number.parseFloat(row[1])))) {
    ...
    if (
      stops.length &&
      stops[0].coordinates &&
      Number.isNaN(stops[0].coordinates.lat) &&
      Number.isNaN(stops[0].coordinates.lon)
    ) {
